...
const RatingPopup = ({ thumbnail, title, setIsRatingPopupOpen }) => {
  ...
  const onOverlayClick = (e) => {
	...
	return (
		<div className="ratingPopup" onClick={(e) => onOverlayClick(e)}>
        ...
        <div className="ratingPopup__buttonGroup">
				  <button className="ratingPopup__cancelButton" onClick={() => setIsRatingPopupOpen(false)}>CANCEL</button>
          {rating > 0 && <button className="ratingPopup__submit">Rate</button>}
        </div>
			</div>
		</div>
	);
